...
int main() {
    ...
    mbot_motor_set_duty(0, 0.2);
    mbot_motor_set_duty(2, 0.2);
    for(int i=0; i<5; i++){
        printf("E0: %d , E1: %d\n", mbot_encoder_read_delta(0), mbot_encoder_read_delta(2));
        sleep_ms(100);
    }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        mbot_motor_set_duty(2, motor_duties[i][1]);
